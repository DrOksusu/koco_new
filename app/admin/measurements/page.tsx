...
export default function MeasurementsAdminPage() {
  ...
  const [editForm, setEditForm] = useState<Partial<MeasurementDefinition>>({});
  ...
  const fetchDefinitions = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(`${data.count}개의 계측값 정의를 불러왔습니다`);
      } else {
  ...
  // Start editing
  const startEdit = (def: MeasurementDefinition) => {
    setEditingId(def.id);
    setEditForm(def);
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/measurements/${editingId}`, {
        method: 'PUT',
        headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">계측값 정의 관리</h2>
        ...
        <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded">
          <p className="text-sm text-yellow-800">
            <strong>참고:</strong> 계측값 정의를 수정하면 모든 사용자에게 즉시 반영됩니다.
            신중하게 수정해 주세요.
          </p>
        </div>
      </div>
